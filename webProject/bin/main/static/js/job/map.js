...
const jobSearch = () => {
    let joservice = $("select[id=joserviceSelect] option:selected").text();   // 선택된 option의 text를 받아옴

    fetch(`/joboffer/map.do?joservice=${joservice}`, {method : 'GET'})
    .then(r => {
        if (!r.ok) {  // 상태 코드가 200번대가 아니면 에러 처리
            throw new Error(`HTTP error! status: ${r.status}`);
        }
        return r.json();  // JSON 응답 처리
    })
    .then(d => {
        console.log(d);
    ...
    .catch(e => {
        ...
        if (e.message.includes("404")) {
            alert("해당 카테고리의 구인 공고가 없습니다.");  // 404 에러가 발생했을 때 사용자에게 알림
        } else {
            alert("서버 오류가 발생했습니다. 다시 시도해주세요.");  // 기타 오류 처리
        }
    });
}
